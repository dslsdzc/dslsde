...
impl InferenceEngine {
    ...
    pub(crate) fn make_assign(&self, addr: u64, regs: &mut HashMap<String, ValueDomain>, stack: &mut HashMap<i64, ValueDomain>, dst: &str, src: &str) -> Stmt {
        ...
        if let Some(off) = so(src) { if let Some(d) = ro(dst) { let val = stack.get(&off).cloned().unwrap_or(ValueDomain::Unknown); regs.insert(d.to_string(), val.clone()); // 保留原始操作数（含数组索引）
            let info = if src.contains('*') { src.to_string() } else { format!("[rbp{:+}]", off) };
            return Stmt::Assign { addr, dst: d.to_string(), val, info, anno: Annotation::None }; } return Stmt::Nop; }
        if let Some(d) = ro(dst) { if let Some(v) = iv(src) { let val = ValueDomain::Signed(v); regs.insert(d.to_string(), val.clone()); return Stmt::Assign { addr, dst: d.to_string(), val, info: src.to_string(), anno: Annotation::None }; } if let Some(s) = ro(src) { let val = regs.get(s).cloned().unwrap_or(ValueDomain::Unknown); regs.insert(d.to_string(), val.clone()); return Stmt::Assign { addr, dst: d.to_string(), val, info: s.to_string(), anno: Annotation::None }; } if src.contains('[') && !src.contains("rbp") && !src.contains("rip") { regs.insert(d.to_string(), ValueDomain::Unknown); return Stmt::Assign { addr, dst: d.to_string(), val: ValueDomain::Unknown, info: src.to_string(), anno: Annotation::None }; } }
        if dst.contains('[') && !dst.contains("rbp") && !dst.contains("rip") { if let Some(s) = ro(src) { let val = regs.get(s).cloned().unwrap_or(ValueDomain::Unknown); return Stmt::Assign { addr, dst: dst.to_string(), val, info: s.to_string(), anno: Annotation::None }; } if let Some(v) = iv(src) { return Stmt::Assign { addr, dst: dst.to_string(), val: ValueDomain::Signed(v), info: src.to_string(), anno: Annotation::None }; } return Stmt::Assign { addr, dst: dst.to_string(), val: ValueDomain::Unknown, info: src.to_string(), anno: Annotation::None }; }
        Stmt::Nop
    }
